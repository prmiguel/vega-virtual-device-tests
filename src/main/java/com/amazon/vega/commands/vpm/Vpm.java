package com.amazon.vega.commands.vpm;

import com.amazon.vega.VegaDriver;
import com.amazon.vega.commands.VegaCommand;

public abstract class Vpm<T> implements VegaCommand {

    protected final VegaDriver driver;
    protected String command;
    protected String result;

    protected Vpm(VegaDriver driver) {
        this.driver = driver;
    }

    public T execute() {
        this.result = String.valueOf(driver.executeScript("shell", String.format("vpm %s", this.command)));
        return transformResult(this.result);
    }

    protected abstract T transformResult(String result);
}
